...
from FinalSN import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"FinalSN/plugins/{shortname}.py")
        checkplugins(path)
        name = "FinalSN.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"FinalSN/plugins/{shortname}.py")
            name = f"FinalSN.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["FinalSN.plugins." + shortname] = mod
        LOGS.info("⌁⌁︙تم بنجاح تحميل ملف ✓" + shortname)
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    final313.remove_event_handler(i)
    ...
    try:
        name = f"FinalSN.plugins.{shortname}"
        for i in reversed(range(len(final313._event_builders))):
            ev, cb = final313._event_builders[i]
            if cb.__module__ == name:
                del final313._event_builders[i]
    except BaseException:
        raise ValueError
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"FinalSN/plugins/assistant/{shortname}.py")
        name = "FinalSN.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["FinalSN.plugins.assistant" + shortname] = mod
