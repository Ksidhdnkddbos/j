        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                if hasattr(chat, "title"):
                    if( "ALvipfinal" in     chat.title and not (chat.admin_rights or chat.creator) and not (check.sender_id in DEVJOKR)
                    ):
                        await edit_delete(check, "** ⌁⌁︙ لا يمكنني استخدام سورس فـاينل هنا في هذه المجموعة 🤷🏻 **")
                        return
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**تقرير خطا فـاينل**\n\n"
                        link = "[هنا](https://t.me/Z3ZZ_ZSupport)"
            ...
            if pattern is not None:
                ...
                if edited:
                    final313.add_event_handler(func, events.MessageEdited(**kwargs))
                final313.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                final313.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                final313.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
