...
def main_menu():
    text = f"**- [سـورس فـاينل](https://t.me/Z3ZZ_Z)\
        \n المسـاعد\
        \n\
        \n◽ فـاينل لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("final313"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" FinalSN UsᴇʀBoᴛ", "https://t.me/Z3ZZ_Z"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./FinalSN", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس فـاينل 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./FinalSN", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس فـاينل 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./FinalSN", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس فـاينل 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© FinalSN Help",
                description="Help menu for FinalSN",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Z3ZZ_Z"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/Z3ZZ_Z1",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت فـاينل 🧸♥](https://t.me/Z3ZZ_Z)**\
            \n\
            \n❤ فـاينل هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/Z3ZZ_Z1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@final313.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@final313.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@final313.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
