...
def getBannerLink(mal, kitsu_search=True, anilistid=0):
    ...
    response = requests.post("https://graphql.anilist.co", json=data).json()
    if "data" in response and "Media" in response["data"] and "bannerImage" in response["data"]["Media"]:
        image = response["data"]["Media"]["bannerImage"]
        return image
