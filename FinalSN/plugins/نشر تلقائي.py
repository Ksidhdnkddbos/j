...
@final313.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        FinalSN = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        FinalSN = str(jok).replace("-100", "")
    else:
        try:
            FinalSN = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await final313.send_message(int(chat), event.message)
