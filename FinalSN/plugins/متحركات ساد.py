...
#جميع الحقوق محفوظة لسـورس فـاينل تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    i0i0ii = await reply_id(event)
    if sad:
        final313 = f"**˛ JEP ، ٰ𝖦𝗂𝖿 𝖲ِ𝖺ٰ𝖣 . .**\n"
        final313 += f"✛━━━━━━━━━━━━━✛\n"
        final313 += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=final313, reply_to=i0i0ii)
