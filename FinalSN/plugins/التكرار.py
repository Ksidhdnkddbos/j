...
@final313.ar_cmd(pattern="تكرار الملصق$")
...
@final313.ar_cmd(pattern="ايقاف التكرار ?(.*)")
...
#جميع الاكواد ادناه تمت كتابتها من قبل مطورين فـاينل ممنوع السرقة !
...
async def ffinal(event):
    ...
    for chat_username in chat_usernames:
        try:
            ...
            await alvipfinal_nshr(final313, seconds, chat.id, message, seconds)  # تمرير قيمة seconds هنا لكل مجموعة
...
async def alvipfinal_allnshr(client, sleeptimet, message):
    ...
    while yaAli:
        for chat in alvipfinal_chats:
            if chat.is_group and chat.title != "فريق فـاينل •Team ALvipfinal":
                try:
                    if message.media:
...
@final313.ar_cmd(pattern="نشر_كروبات")
async def ffinal(event):
    await event.delete()
...
@final313.ar_cmd(pattern="ايقاف (النشر|نشر)")
