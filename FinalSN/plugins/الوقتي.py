...
from FinalSN import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, final313, logging
...
autopic_path = os.path.join(os.getcwd(), "FinalSN", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "FinalSN", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "FinalSN", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "FinalSN", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "FinalSN", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "FinalSN/helpers/styles/PaybAck.ttf"
...
vipfinal_timezone = pytz.timezone('Asia/Baghdad')

async def alvipfinal_timezone(event, tz_name):
    global vipfinal_timezone
    if tz_name in TIMEZONES:
        vipfinal_timezone = pytz.timezone(TIMEZONES[tz_name])
...
@final313.on(admin_cmd(pattern="وقت (.+)"))
async def handle_timezone(event):
    tz_name = event.pattern_match.group(1).strip()
    await alvipfinal_timezone(event, tz_name)
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        current_time = datetime.now(vipfinal_timezone).strftime("%I:%M")
            ...
            if i > 0:
                await final313(
                    functions.photos.DeletePhotosRequest(
                        await final313.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            result = await final313(functions.photos.UploadProfilePhotoRequest(file=await final313.upload_file(autophoto_path)))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await final313.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in final313.iter_profile_photos(int(dgp), limit=1) :
                    await final313(
            ...
            i += 1
            await final313(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await final313.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        time.strftime("%d-%m-%y")
        HM = datetime.now(vipfinal_timezone).strftime("%I:%M")
        for normal in HM:
        ...
        try:
            await final313(functions.channels.EditTitleRequest(
                channel=await final313.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await final313.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
    ...
    while AUTONAMESTART:
        ...
        for normal in HM:
            if normal in normzltext:
                ...
                namefont = namerzfont[normzltext.index(normal)]
                HM = HM.replace(normal, namefont)
                i0i0ii = gvarstatus("TIME_JEP") or ""
        name = f"{i0i0ii} {HM}"
        LOGS.info(name)
        try:
            await final313(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        time.strftime("%d.%m.%Y")
        HI = datetime.now(vipfinal_timezone).strftime("%I:%M")
        for normal in HI:
            if normal in normzltext:
                namerzfont = gvarstatus("JP_FN") or "𝟏𝟐𝟑𝟒𝟓𝟔𝟕𝟖𝟗𝟎"
                namefont = namerzfont[normzltext.index(normal)]
        ...
        LOGS.info(bio)
        try:
            await final313(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@final313.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@final313.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@final313.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@final313.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@final313.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
final313.loop.create_task(digitalpicloop())
final313.loop.create_task(digitalgrouppicloop())
final313.loop.create_task(autoname_loop())
final313.loop.create_task(autobio_loop())
final313.loop.create_task(group_loop())