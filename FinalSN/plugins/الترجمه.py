...
import json
import os
from FinalSN.helpers.functions.functions import translate
...
@final313.ar_cmd(pattern="(الترجمة الفورية|الترجمه الفوريه|ايقاف الترجمة|ايقاف الترجمه)")
async def reda(event):
    if gvarstatus("transnow"):
        delgvar("transnow")
        await edit_delete(event, "**⌁⌁︙ تم تعطيل الترجمه الفورية **")
    else:
        addgvar("transnow", "F16") 
        await edit_delete(event, "**⌁⌁︙ تم تفعيل الترجمه الفورية**")

@final313.ar_cmd(pattern="لغة الترجمة")
