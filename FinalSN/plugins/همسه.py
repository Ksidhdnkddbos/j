...
async def wspr(event):
    if event.fwd_from:
        return
    final313b = event.pattern_match.group(1)
    rrrd7 = "@nnbbot"
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    tap = await bot.inline_query(rrrd7, final313b) 
...
# كتابة وتعديل فريق فـاينل  #@i0i0ii
