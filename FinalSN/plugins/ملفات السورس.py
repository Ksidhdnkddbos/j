...
@final313.ar_cmd(pattern="المكاتب")
...
@final313.ar_cmd(
    ...
    info={
        "header": "To list all plugins in FinalSN.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in FinalSN"
    cmd = "ls FinalSN/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[فـاينل](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@final313.ar_cmd(
    ...
    info={
        "header": "To list all environment values in FinalSN.",
        "description": "to show all heroku vars/Config values in your FinalSN",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in FinalSN"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[فـاينل](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
    ...
    await final313.send_message("me", OUTPUT)
...
@final313.ar_cmd(pattern="رابط مباشر")
