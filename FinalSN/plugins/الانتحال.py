# Copyright (C) 2021 FinalSN TEAM
# FILES WRITTEN BY  @i0i0ii
import html
import base64
...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @z3zz_z"
)

@final313.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await final313.get_me()
...
#F16
jeps = ["FinalSN", "HuResupport"]
@final313.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await final313(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await final313(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await final313(functions.channels.EditTitleRequest(
                channel=await final313.get_entity(event.chat_id),
        ...
        try:
            await final313(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await final313.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await final313.upload_file(profile_pic)
        try:
            await final313(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @z3zz_z")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
#F16
@final313.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in final313.iter_profile_photos(event.chat_id, limit=1) :
                    await final313(
