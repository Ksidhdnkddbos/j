from FinalSN import final313, bot
import time
...
JOKER_PIC = "https://telegra.ph/file/fde4f0530c198573ea760.jpg"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        vipfinal = Bot_Username.replace("@", "")
        query = event.text
        await bot.get_me()
        if query.startswith("هاك") and event.query.user_id == bot.uid:
            buttons = Button.url("• اضغط هنا عزيزي •", f"https://t.me/{vipfinal}")
...
async def repo(event):
    if event.fwd_from:
        return
    i0i0ii = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    await bot.send_message(i0i0ii, "/hack")
    response = await bot.inline_query(i0i0ii, "هاك")
    await response[0].click(event.chat_id)
    await event.delete()

@final313.ar_cmd(pattern="اشتراك")
...
@final313.ar_cmd(incoming=True)
async def reda(event):
    try:
        sender = await event.get_sender()
    except ChannelPrivateError:
        return
    if isinstance(sender, types.User) and (sender.bot or sender.id in {5434703779, 777000}):
        return
    if gvarstatus("subprivate"):
        if event.is_private:
            try:
                idd = event.peer_id.user_id
                if idd == 5434703779 and not gvarstatus("developer_alvipfinal"):
                    addgvar("developer_alvipfinal", True)
                    await event.reply("اهلا مطوري العزيز 🖤")
                else:
                    tok = Config.TG_BOT_TOKEN
                    ch = gvarstatus("pchan")
                    if not ch:
                        return await final313.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                    try:
                        ch = int(ch)
                    except BaseException as r:
                        return await final313.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                    url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                    req = requests.get(url)
                    reqt = req.text
                    if "chat not found" in reqt:
                        mb = await final313.tgbot.get_me()
                        mb = mb.username
                        await final313.tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                        return
                    if "bot was kicked" in reqt:
                        mb = await final313.tgbot.get_me()
                        mb = mb.username
                        await final313.tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                        return
                    if "not found" in reqt:
                        try:
                            c = await final313.get_entity(ch)
                            chn = c.username
                            if c.username == None:
                                ra = await final313.tgbot(ExportChatInviteRequest(ch))
                                chn = ra.link
                            ...
                            await final313.tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                    if "left" in reqt:
                        try:
                            c = await final313.get_entity(ch)
                            chn = c.username
                    ...
                    if "error_code" in reqt:
                        await final313.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@i0i0ii لحل المشكلة\n{reqt}**")
