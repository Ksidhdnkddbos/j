...
vipfinal_users = []
vipfinal_mute = "https://telegra.ph/file/9edfc45674dbf908b1125.jpg"
vipfinal_unmute = "https://telegra.ph/file/acb252dedd25dbe7d672d.jpg"
import os

file_path = 'finalvMute.txt'
...
@final313.ar_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)")
...
@final313.ar_cmd(pattern=f"(الغاء الكتم|الغاء كتم)(?:\s|$)([\s\S]*)")
...
@final313.ar_cmd(pattern=r"قائمة المكتومين")
async def show_muted_users(event):
    if os.path.isfile(file_path):
        with open(file_path, 'r') as file:
            muted_ids = file.read().splitlines()
        if len(muted_ids) > 0:
            vipfinal_list = "**⌁⌁︙ قائمة المستخدمين المكتومين:**\n"
            for i, user_id in enumerate(muted_ids, start=1):
                try:
                    user = await event.client.get_entity(int(user_id))
                    vipfinal_link = f"[{user.first_name}](tg://user?id={user.id})"
                    vipfinal_list += f"{i}. {vipfinal_link}\n"
                except ValueError:
                    vipfinal_list += f"{i}. User ID: {user_id} (Error: Could not find user)\n"
                except Exception as e:
                    vipfinal_list += f"{i}. User ID: {user_id} (Error: {e})\n"
            await event.edit(vipfinal_list)
        else:
