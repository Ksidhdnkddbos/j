...
async def save_media(event):
    ...
    try:
        message = await final313.get_messages(int(channel_id), ids=int(message_id))
        if not message:
        ...
        if message.media or message.document:
            ...
            await final313.send_message(event.chat_id, file_ext)
            file_path = os.path.join(save_dir, f"media_{file_ext}")
            await final313.download_media(message, file=file_path)

            await final313.send_file('me', file=file_path, caption=message.text)
...
async def ffinal(event):
    ...
    try:
        channel_entity = await final313.get_entity(channel_username)
        messages = await final313.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await final313.send_file("me", file=file_path)
                os.remove(file_path)
...
async def _(event):
    ...
    for i in [inputfile, outputfile]:
        if os.path.exists(i):
            os.remove(i)
#write Code By #ffinal For FINAL ☠️
@final313.ar_cmd(
    pattern=r"بنتيرست (.+)",
    command=("بنتيرست", plugin_category),
)
async def pinterestfinalv(event):
