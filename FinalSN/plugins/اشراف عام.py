#    جميع الحقوق محفوظة كتابة وتعديل  :   @i0i0ii
...
from FinalSN import CMD_HELP, final313
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @z3zz_z", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@final313.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(FinalSN):
    dc = razan = FinalSN
    i = 0
    sender = await dc.get_sender()
    me = await FinalSN.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await FinalSN.get_chat()
    if FinalSN.is_private:
        user = FinalSN.chat
        rank = FinalSN.pattern_match.group(1)
    else:
        FinalSN.chat.title
    try:
        user, rank = await get_full_user(FinalSN)
    except:
        pass
    if me == user:
       final313 = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await FinalSN.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await FinalSN.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@final313.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(FinalSN):
    dc = razan = FinalSN
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       final313 = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
