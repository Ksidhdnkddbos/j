...
from final313.razan.resources.assistant import *
from final313.razan.resources.mybot import *
from FinalSN import final313
from ..core import check_owner
from ..Config import Config

JEP_IC = "https://telegra.ph/file/8dc1ca64cad67d4becf28.jpg"
ROE = "**♰ هـذه هي قائمة اوامـر سـورس فـاينل ♰**"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == bot.uid:
            buttons = [
                [Button.inline("⌁⌁ اوامر الادمن ⌁⌁", data="final3130")],
                [
...
async def repo(event):
    ...
    response = await bot.inline_query(i0i0ii, "اوامري")
    await response[0].click(event.chat_id)
    await event.delete()


@final313.tgbot.on(CallbackQuery(data=re.compile(rb"final3130")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"jrzst")))
@check_owner
async def _(event):
    butze = [
    [
     Button.inline("التالي", data="tslrzj"),
     Button.inline("رجوع", data="final3130")]]
    await event.edit(GRTSTI, buttons=butze)


@final313.tgbot.on(CallbackQuery(data=re.compile(rb"tslrzj")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"krrznd")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"rozbot")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"Jmrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"r7brz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"sejrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"gro")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"grrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"iiers")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"rfhrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@final313.tgbot.on(CallbackQuery(data=re.compile(rb"uscuxrz")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="final3130"),]]
