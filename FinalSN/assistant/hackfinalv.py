...
from FinalSN import bot, final313
#By Source vipfinal @z3zz_z
from telethon import events, functions, types, Button
from datetime import timedelta
from FinalSN.utils import admin_cmd
import asyncio
...
'''
mm = '''
قم بلأنضمام الى قناة فـاينل @Z3ZZ_Z
'''
...
keyboard = [
  ...
  [
    Button.url("سورس فـاينل ☠️", "https://t.me/Z3ZZ_Z")
    ]
]
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("هاك") and event.query.user_id == bot.uid:
            buttons = Button.url(" اضغط هنا عزيزي ", f"https://t.me/{vipfinal}?start=hack")
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/Z3ZZ_Z")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس فـاينل", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك فـاينل.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك فـاينل.", buttons=keyboard)
      else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/Z3ZZ_Z")
        ]
    ]
